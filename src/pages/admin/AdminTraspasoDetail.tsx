...
import {
  ...
  Timer, TrendingUp, AlertTriangle, Sparkles, UserPlus,
} from "lucide-react";
...
export default function AdminTraspasoDetail() {
  ...
  const { data: traspaso, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      setSelectedGestorId((data as any).gestor_id || "");
      setSelectedNotarioId((data as any).notario_id || "");
      setSelectedMensajeroId((data as any).mensajero_id || "");
      return data;
    },
  });

  // Fetch profiles by role for assignment dropdowns
  const { data: gestores = [] } = useQuery({
    queryKey: ["profiles-gestor"],
    queryFn: async () => {
      const { data } = await supabase.from("profiles").select("id, nombre, email").eq("role", "gestor");
      return data || [];
    },
  });
  const { data: notarios = [] } = useQuery({
    queryKey: ["profiles-notario"],
    queryFn: async () => {
      const { data } = await supabase.from("profiles").select("id, nombre, email").eq("role", "notario");
      return data || [];
    },
  });
  const { data: mensajeros = [] } = useQuery({
    queryKey: ["profiles-mensajero"],
    queryFn: async () => {
      const { data } = await supabase.from("profiles").select("id, nombre, email").eq("role", "mensajero");
      return data || [];
    },
  });
      ...
      <div className="container py-6 grid lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          ...
          <Card>
            ...
            <CardContent className="text-sm space-y-1">
              ...
              {traspaso.medio_pago && (
                <p><span className="text-muted-foreground">Medio de Pago:</span> {traspaso.medio_pago}</p>
              )}
          ...
          <div className="grid grid-cols-2 gap-4">
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm flex items-center gap-1"><User className="h-3.5 w-3.5" /> Vendedor</CardTitle>
              </CardHeader>
              <CardContent className="text-sm space-y-1">
                <p>{traspaso.vendedor_nombre}</p>
                <p className="text-muted-foreground">
                  {traspaso.vendedor_tipo_persona === "juridica" ? `RNC: ${traspaso.vendedor_rnc || "—"}` : `Cédula: ${traspaso.vendedor_cedula || "—"}`}
                </p>
                {traspaso.vendedor_tipo_persona === "juridica" && <Badge variant="secondary" className="text-xs">Empresa</Badge>}
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm flex items-center gap-1"><User className="h-3.5 w-3.5" /> Comprador</CardTitle>
              </CardHeader>
              <CardContent className="text-sm space-y-1">
                <p>{traspaso.comprador_nombre}</p>
                <p className="text-muted-foreground">
                  {traspaso.comprador_tipo_persona === "juridica" ? `RNC: ${traspaso.comprador_rnc || "—"}` : `Cédula: ${traspaso.comprador_cedula || "—"}`}
                </p>
                {traspaso.comprador_tipo_persona === "juridica" && <Badge variant="secondary" className="text-xs">Empresa</Badge>}
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-base flex items-center gap-2">
                ...
                <Badge className={antifraudeStyle(traspaso.antifraude_status)} variant="secondary">{traspaso.antifraude_status}</Badge>
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="border rounded-lg overflow-hidden bg-muted aspect-[3/4] flex items-center justify-center">
                    {selfieComprador ? <img src={selfieComprador.file_url} alt="Selfie comprador" className="object-cover w-full h-full" /> : <p className="text-xs text-muted-foreground">Sin selfie</p>}
                  </div>
                  <div className="border rounded-lg overflow-hidden bg-muted aspect-[3/4] flex items-center justify-center">
                    {cedulaComprador ? <img src={cedulaComprador.file_url} alt="Cédula comprador" className="object-cover w-full h-full" /> : <p className="text-xs text-muted-foreground">Sin cédula</p>}
                  </div>
                </div>
                {selfieComprador && cedulaComprador && (
                  <Button size="sm" variant="outline" className="w-full mt-2" disabled={aiVerifying === "comprador"}
                    onClick={() => runAiVerification("comprador", selfieComprador.file_url, cedulaComprador.file_url)}>
                    {aiVerifying === "comprador" ? <><Loader2 className="h-4 w-4 mr-1 animate-spin" /> Verificando…</> : <><Sparkles className="h-4 w-4 mr-1" /> Verificar con AI</>}
                  </Button>
                )}
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="border rounded-lg overflow-hidden bg-muted aspect-[3/4] flex items-center justify-center">
                    {selfieVendedor ? <img src={selfieVendedor.file_url} alt="Selfie vendedor" className="object-cover w-full h-full" /> : <p className="text-xs text-muted-foreground">Sin selfie</p>}
                  </div>
                  <div className="border rounded-lg overflow-hidden bg-muted aspect-[3/4] flex items-center justify-center">
                    {cedulaVendedor ? <img src={cedulaVendedor.file_url} alt="Cédula vendedor" className="object-cover w-full h-full" /> : <p className="text-xs text-muted-foreground">Sin cédula</p>}
                  </div>
                </div>
                {selfieVendedor && cedulaVendedor && (
                  <Button size="sm" variant="outline" className="w-full mt-2" disabled={aiVerifying === "vendedor"}
                    onClick={() => runAiVerification("vendedor", selfieVendedor.file_url, cedulaVendedor.file_url)}>
                    {aiVerifying === "vendedor" ? <><Loader2 className="h-4 w-4 mr-1 animate-spin" /> Verificando…</> : <><Sparkles className="h-4 w-4 mr-1" /> Verificar con AI</>}
                  </Button>
                )}
              ...
              <div>
                ...
                <Textarea value={antifraudeNotas} onChange={(e) => setAntifraudeNotas(e.target.value)} placeholder="Observaciones sobre la verificación..." className="mt-1" rows={2} />
              </div>
          ...
          {docs && docs.length > 0 && (
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm flex items-center gap-2"><FileText className="h-4 w-4" /> Todos los Documentos</CardTitle>
              </CardHeader>
              <CardContent>
        ...
        <div className="space-y-4">
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-base flex items-center gap-2">
                <UserPlus className="h-4 w-4 text-accent" /> Asignar Roles
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
              <div>
                ...
                <Select value={selectedGestorId} onValueChange={setSelectedGestorId}>
                  ...
                  <SelectContent>
                    ...
                    {gestores.map((g: any) => (
                      <SelectItem key={g.id} value={g.id}>{g.nombre || g.email || g.id}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
              <div>
                <Label className="text-xs">Notario</Label>
                <Select value={selectedNotarioId} onValueChange={setSelectedNotarioId}>
                  <SelectTrigger><SelectValue placeholder="Seleccionar notario..." /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="none">Sin asignar</SelectItem>
                    {notarios.map((n: any) => (
                      <SelectItem key={n.id} value={n.id}>{n.nombre || n.email || n.id}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
              <div>
                <Label className="text-xs">Mensajero</Label>
                <Select value={selectedMensajeroId} onValueChange={setSelectedMensajeroId}>
                  <SelectTrigger><SelectValue placeholder="Seleccionar mensajero..." /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="none">Sin asignar</SelectItem>
                    {mensajeros.map((m: any) => (
                      <SelectItem key={m.id} value={m.id}>{m.nombre || m.email || m.id}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
              <Button
                variant="outline"
                className="w-full"
                onClick={() => updateTraspaso.mutate({
                  gestor_id: selectedGestorId && selectedGestorId !== "none" ? selectedGestorId : null,
                  notario_id: selectedNotarioId && selectedNotarioId !== "none" ? selectedNotarioId : null,
                  mensajero_id: selectedMensajeroId && selectedMensajeroId !== "none" ? selectedMensajeroId : null,
                } as any)}
              >
                ...
                return (
                  <div key={s.key} className="flex gap-3">
                    ...
                    <div className="pb-4">
                      ...
                      {entry && <p className="text-[10px] text-muted-foreground">{new Date(entry.created_at).toLocaleString("es-DO")}</p>}
                    </div>
                  </div>
                );
              })}
          ...
          {timeline && timeline.length > 0 && (() => {
            const sortedTimeline = [...timeline].sort((a: any, b: any) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime());
            const stageKpis = sortedTimeline.map((entry: any, idx: number) => {
              const prevTime = idx === 0 ? new Date(traspaso.created_at).getTime() : new Date(sortedTimeline[idx - 1].created_at).getTime();
              const currentTime = new Date(entry.created_at).getTime();
              const durationHours = (currentTime - prevTime) / (1000 * 60 * 60);
              const slaHours = (slaConfig && slaConfig[entry.status]) || 24;
              const isOverdue = durationHours > slaHours;
              return { status: entry.status, durationHours, slaHours, isOverdue };
            });
            ...
            const lastTimelineTime = sortedTimeline.length > 0 ? new Date(sortedTimeline[sortedTimeline.length - 1].created_at).getTime() : new Date(traspaso.created_at).getTime();
            ...
            const statusLabel = (key: string) => STATUS_STEPS.find(s => s.key === key)?.label || key.replace(/_/g, " ");
                ...
                <CardContent className="space-y-4">
                  <div className={`rounded-lg p-3 ${overallOnTime ? "bg-green-50 border border-green-200" : "bg-red-50 border border-red-200"}`}>
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-xs text-muted-foreground">Total: {formatDuration(totalHours)} / {formatDuration(totalSla)} SLA</span>
                    </div>
                  </div>
          ...
          {traspaso.escrow_status !== "no_aplica" && (
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm flex items-center gap-2"><Lock className="h-4 w-4" /> Escrow / Pago Seguro</CardTitle>
              </CardHeader>
              <CardContent className="space-y-3">
                <p className="text-lg font-bold">RD$ {Number(traspaso.precio_vehiculo || 0).toLocaleString()}</p>
                <Select value={traspaso.escrow_status} onValueChange={(v) => updateTraspaso.mutate({ escrow_status: v })}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
          ...
          <Card>
            <CardContent className="pt-4">
              <ContractGenerator
                ...
                contractData={{
                  vehiculo_marca: traspaso.vehiculo_marca || "", vehiculo_modelo: traspaso.vehiculo_modelo || "",
                  vehiculo_ano: traspaso.vehiculo_ano || "", vehiculo_placa: traspaso.vehiculo_placa || "",
                  vehiculo_color: traspaso.vehiculo_color || "", vehiculo_chasis: traspaso.vehiculo_chasis || "",
                  tipo_vehiculo: traspaso.tipo_vehiculo || "vehiculo_motor",
                  vendedor_nombre: traspaso.vendedor_nombre || "", vendedor_cedula: traspaso.vendedor_cedula || "",
                  vendedor_rnc: traspaso.vendedor_rnc || "", vendedor_tipo_persona: traspaso.vendedor_tipo_persona || "fisica",
                  vendedor_telefono: traspaso.vendedor_telefono || "",
                  comprador_nombre: traspaso.comprador_nombre || "", comprador_cedula: traspaso.comprador_cedula || "",
                  comprador_rnc: traspaso.comprador_rnc || "", comprador_tipo_persona: traspaso.comprador_tipo_persona || "fisica",
                  ...
                  medio_pago: traspaso.medio_pago || "", fecha_acto_venta: traspaso.fecha_acto_venta || "",
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              <Textarea value={notasInternas} onChange={(e) => setNotasInternas(e.target.value)} placeholder="Notas internas..." rows={3} />
              <Button size="sm" variant="outline" className="w-full" onClick={() => updateTraspaso.mutate({ notas_internas: notasInternas })}>
                Guardar Notas
              </Button>
            </CardContent>
          </Card>
